...
Reference:
    Viswanath, A., J. Forrester, A. I., & Keane, A. J. (2011). Dimension reduction 
    for aerodynamic design optimization. AIAA journal, 49(6), 1256-1266.
...
if __name__ == "__main__":
    ...
    np.save('opt_results/nurbs_bo/opt_airfoil.npy', opt_airfoil_runs)
    np.save('opt_results/nurbs_bo/opt_history.npy', opt_perfs_runs)
...
#    plt.xticks(np.linspace(0, n_eval+1, 5, dtype=int))
    plt.savefig('opt_results/nurbs_bo/opt_history.svg')
    plt.close()
    ...
    plt.axis('equal')
    plt.savefig('opt_results/nurbs_bo/opt_airfoil.svg')
    plt.close()

    print 'NURBS-BO completed :)'
